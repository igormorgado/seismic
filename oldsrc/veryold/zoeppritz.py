...
def calculate_matrix(phi0,alpha1,beta1,rho1,alpha2,beta2,rho2):
    ...
    veld=np.zeros(4,dtype="complex")
    veln=np.zeros(4,dtype="float")
    ...
    for i in range(4):
        ...
        veld[i]=np.linalg.det(m)
        veln[i]=np.linalg.norm(veld[i])/detm_norm
        phase[i]=np.math.atan2(veld[i].real*detm.real-veld[i].imag*detm.imag,
                               veld[i].real*detm.real+veld[i].imag*detm.imag )
        
        # Copy the vector back    
        m[i]=cvt

    # a1,b1,a2,b3
    return veld, veln, phase        
