...
class Source(object):
    ...
    By definition a seismic source has the following attributes:
        ...
        x (int):    Source position in X coordinate
        y (int):    Source position in Y coordinate
        z (int):    Source position in Z coordinate
        sample_rate (float):  Sampling rate in miliseconds
    """
    __metaclass__ = abc.ABCMeta

    def __init__(self, x, y, z, sample_rate):
        self.x = x
        self.y = y
        self.z = z
...
class Ricker(Source):
    ...
    def __init__(self, central_frequency=None, cut_frequency=None, sample_rate=None):
        ...
        if sum(i is not None for i in locals().values()) > 2:
            raise ValueError("You need to specify only one parameter")
        if central_frequency is not None:
            ...
            raise ValueError("You must define 'central_frequency', 'cut_frequency' or 'sample_rate'")
...
class Dataset(Source):
    """Load a disk/memory dataset as a wave source."""
